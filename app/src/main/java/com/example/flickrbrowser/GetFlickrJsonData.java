...
public class GetFlickrJsonData extends AsyncTask<String, Void, List<Photo>> implements GetRawData.OnDownloadComplete {
    ...
    public GetFlickrJsonData(String baseURL, String language, boolean matchAll, OnDataAvailable callBack) {
        Log.d(TAG, "GetFlickrJsonData: called GetFlickrJsonData class constructor");
        mBaseURL = baseURL;
        mLanguage = language;
        mMatchAll = matchAll;
        mCallBack = callBack;
    ...
    void executeOnSameThread(String searchCriteria) {
        Log.d(TAG, "executeOnSameThread: Thread starts");
    ...
    public void onDownloadComplete(String data, DownloadStatus status) {
        ...
        if (status == DownloadStatus.OK) {
            ...
            try {
                ...
                //json ima polje items složeno u Array. Sa stranice Flickera &format=json
                JSONArray itemsArray = jsonData.getJSONArray("items");

                for (int i = 0; i < itemsArray.length(); i++) {
                    JSONObject jsonPhoto = itemsArray.getJSONObject(i);
                    //string imeString = JSONObject.dohvatiString("naslov"-JSON verzija stranice flickr public feed-a u ITEMS arrayu ima taj naziv koji nas zanima)
        ...
        if (runningOnSameThread && mCallBack != null) {
            //now inform the caller that processsing is done -
